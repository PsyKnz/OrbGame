...
public class UIElement extends InputAdapter {
	...
	private final Array<GameMessage> messageStack;	// Stack of all messages generated by this screen.
	
	private BitmapFont uiFont;		// Font used to draw all UI items.
	private Sprite baseSprite;		// Sprite used to derive all other sprites in the UI.
	...
	public void setCamera(OrthographicCamera camera) {
		...
		for(GameMessage msg : messageStack) msg.setBounds(camera.position.x - camera.viewportWidth / 2,
	...
	public boolean update(float delta) {
		...
		if(displayScore < screen.scores.score) {											// If the score displayed is less than the actual score,
			...
			if(displayScore > screen.scores.getHighscore(0)) {										// If the display score is also greater than the top score,
				...
				scoreBars.highscoreVal.color = Color.GREEN;											// Its color is set to green to indicate the player has beaten the highscore.
			}
		}
	...
	public void displayMessage(String text) {
		...
		GameMessage msg = new GameMessage(msgBackgroundSpr, message, this);
		messageStack.add(msg);
		input.addProcessor(0, msg);
		
		if(camera != null) msg.setBounds(camera.position.x - camera.viewportWidth / 2,
				camera.position.y - camera.viewportHeight / 2, camera.viewportWidth, camera.viewportHeight);
	}
	...
	public void enableInput() {
		...
		TextElement inputMessage = new TextElement("Tap screen to play", uiFont, 0, 0);
		inputMessage.color.a = 0;
		Tween.to(inputMessage.color, ColorTween.COLOR_ALPHA, 0.75f).target(1).repeatYoyo(-1, 0).start(manager);
		messageStack.peek().enableInput(inputMessage);
		
		if(camera != null) messageStack.peek().setBounds(camera.position.x - camera.viewportWidth / 2,
				camera.position.y - camera.viewportHeight / 2, camera.viewportWidth, camera.viewportHeight);;
	}
	
	public void removeMessage(GameMessage message) {
		input.removeProcessor(message);
		messageStack.removeValue(message, true);
	}
